...
class Trainer():
    ...
    def train(self):
        label_txt = open("integrated_label.txt" , "r")
        ...
        for batch, (lr, hr, image_names) in enumerate(self.loader_train):
            ...
            SRImage_path = os.path.join(image_root_path, "SR_Image")
            os.makedirs(SRImage_path, exist_ok = True)

            for i in range(batch):
                cv2.imwrite(sr[-1][i], os.path.join(SRImage_path, image_name[i]+'png'))
